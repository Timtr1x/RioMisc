...
实网常见返回（`code` 可能相同，靠 `message` 区分）：

| 情况 | code | message 示例 |
| --- | --- | --- |
| 正确 | `00000` | `""`，且 `data.isCorrect === true` |
| 错误 | `40001` | `提交flag错误，请重新提交（当前还有N次提交机会）` |
| 限流 | `40001` | `请求过于频繁，请稍后重试` |

竞赛规则还约定：flag 外形为 `DASCTF{}` / `flag{}`，但 Agent API 提交时通常只传大括号内的内容。
