...
export class VisualRuntime {
  constructor(private opts: { vision?: VisionModelAdapter | null } = {}) {}
  ...
  async analyze(req: VisualAnalysisRequest, absPath: string, artifactDir: string): Promise<VisualAnalysisResult> {
    ...
    const allowVision = mode === "VISION_MODEL" || (mode === "AUTO" && qrs.length === 0 && req.budget?.allowVisionModel !== false);
    if (mode !== "LOCAL_ONLY" && allowVision && this.opts.vision && qrs.length === 0) {
      try {
